...
<body class="bg-cream text-charcoal antialiased <?= e($bodyClass) ?>">
    ...
    <div class="fixed top-24 right-4 z-[1001] space-y-2 pointer-events-auto" id="flash-messages">
        <?php foreach (\Core\Session::getFlash() as $type => $messages): ?>
            <?php foreach ($messages as $message): ?>
            <div class="flash-message px-4 py-3 rounded-lg shadow-lg max-w-sm transition-opacity duration-300
                <?php
                switch ($type) {
                ...
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" onclick="this.closest('.flash-message').style.opacity='0'; setTimeout(() => this.closest('.flash-message').remove(), 300);" class="ml-4 text-white/70 hover:text-white transition-opacity">
