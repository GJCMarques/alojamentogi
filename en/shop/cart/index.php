    ...
    <div class="relative h-full flex flex-col justify-end max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <nav class="mb-4 animate-on-scroll" data-animation="fade-up">
            <ol class="flex items-center space-x-2 text-sm text-white/90">
                ...
                <li class="text-white font-medium">Cart</li>
            </ol>
        </nav>
        ...
        <p class="inline-block text-accent text-lg font-medium tracking-[0.15em] uppercase mb-4 animate-on-scroll" data-animation="fade-up">
            <?= count($cartItems) ?> <?= count($cartItems) === 1 ? 'product' : 'products' ?> in cart
        </p>
        <?php endif; ?>

        <!-- Title -->
        <h1 class="font-cursive text-4xl md:text-5xl lg:text-6xl text-cream drop-shadow-xl animate-on-scroll" data-animation="fade-up" data-delay="100">Shopping Cart</h1>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-md p-12 lg:p-16 text-center animate-on-scroll" data-animation="fade-up">
            <div class="max-w-md mx-auto">
                ...
                <h2 class="font-serif text-3xl text-granite-800 mb-3">Your cart is empty</h2>
                <p class="text-granite-600 text-lg mb-8">You haven't added any products to your cart yet.</p>
                <a href="<?= $base ?>/en/shop/"
        ...
        <div class="lg:grid lg:grid-cols-3 lg:gap-8">
                    ...
                    <div class="cart-item border-b border-granite-100 last:border-b-0 p-6 hover:bg-cream-50 transition-colors duration-200" data-product-id="<?= $product->id ?>">
                        <div class="md:grid md:grid-cols-12 md:gap-6 md:items-center">
                            ...
                            <div class="col-span-2 flex items-center justify-between md:justify-center mb-4 md:mb-0">
                                <span class="md:hidden text-granite-600 font-medium">Quantity:</span>
            ...
            <div class="lg:col-span-1 mt-8 lg:mt-0 animate-on-scroll" data-animation="fade-up" data-delay="100">
                <div class="bg-white rounded-2xl shadow-md p-6 lg:sticky lg:top-24" id="cart-summary">
                    <h2 class="font-serif text-2xl text-granite-900 mb-6 pb-3 border-b border-granite-200">Order Summary</h2>
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-granite-600">Shipping</span>
                            ...
                            <?php else: ?>
                            <span class="text-olive-600 font-semibold" id="cart-shipping">Free</span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="pt-3 pb-2 px-4 bg-olive-50 rounded-lg border border-olive-200">
                            <p class="text-sm text-olive-800 font-medium mb-2">
                                ...
                                <strong><?= formatPrice($amountToFreeShipping) ?></strong> away from free shipping!
                            </p>
                    ...
                    <div class="border-t-2 border-granite-200 pt-4 mb-6">
                        ...
                        <p class="text-xs text-granite-500 mt-1 text-right">VAT included</p>
                    </div>

                    <a href="<?= $base ?>/en/shop/checkout/"
                    ...
                    <div class="mt-6 pt-6 border-t border-granite-200">
                        <p class="text-sm text-granite-600 font-medium mb-3">Accepted payment methods:</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.remove-item').forEach(btn => {
        btn.addEventListener('click', async function() {
            const productId = this.dataset.productId;
            const productName = document.querySelector(`.cart-item[data-product-id="${productId}"] a.font-medium`)?.textContent?.trim() || 'this product';

            const confirmed = await GiModal.confirm(
                `Are you sure you want to remove "${productName}" from your cart?`,
                'Remove Product',
                { type: 'danger', confirmText: 'Yes, remove', cancelText: 'Cancel' }
            );
    ...
    if (clearCartBtn) {
        clearCartBtn.addEventListener('click', async function() {
            const confirmed = await GiModal.confirm(
                'All products will be removed from your cart. This action cannot be undone.',
                'Clear Cart',
                { type: 'danger', confirmText: 'Yes, clear all', cancelText: 'Cancel' }
            );
